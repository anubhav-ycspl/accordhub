...
class Ph_WC_Email_Booking_Confirmation extends WC_Email {
	...
	public function __construct() {
        ...
        add_action( 'ph_booking_status_changed', array( $this, 'ph_send_booking_confirmation_email' ), 10, 4);
	 ...
	 */
	public function ph_send_booking_confirmation_email($status, $item_id, $order_id, $order=''){

		// Trigger the email sending process with the provided parameters.
		$this->trigger($order_id, $status, $item_id, $order);
	}
	 ...
	 * @return void
	 */
	public function trigger($order_id, $status='', $item_id='', $order='') 
	{
		if ( ! $order_id )
			return;
		...
		if ( is_a( $order, 'WC_Order' ) ) {
			...
			$this->recipient_email 	   	= $this->object->get_billing_email();
		...
		if(!($this->item instanceof WC_Order_Item_Product) || ($this->item->get_product()->get_type() != 'phive_booking') || ($status != 'confirmed'))
		{
			return;
		}
		...
		if ( ! $this->is_enabled() || ! $this->recipient_email )
			return;

		// woohoo, send the email!
		$this->send( $this->recipient_email, $this->get_subject(), $this->get_content(), $this->get_headers(), $this->get_attachments() );
