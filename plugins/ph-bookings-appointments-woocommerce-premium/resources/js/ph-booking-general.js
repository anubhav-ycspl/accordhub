...
function calculate_price() {
	...
	if (firstActiveValue) {
		...
		date_from = firstActiveValue;
	}
	jQuery(".ph-date-from").val(date_from);
	//console.log("Cus4:"+date_from);
...
jQuery(document).ready(
	function ($) {
			...
			while (elem && i < selectionLimit) {
				...
				// Case of first click
					...
					if (i == min_limit && from.hasClass("not-startable")) {
						...
						//select_date( elem, "selected-date cc4" );
				...
				// Need to complite an interval.
					...
					if (booking_start_date.getMonth() != booking_end_date.getMonth() && ($('#calender_type').val() != 'time' && $('#calender_type').val() != 'month')) {
						...
						if (!is_reached_time_period(final_date)) {
							...
							select_date(elem, "selected-date cc5");
							if ($(elem).hasClass('de-active') && is_overridable_by_charge_per_night($(elem))) {
								elem = false;
							} else {
						...
						//select_date( elem, "selected-date cc6" );
						elem = false;
					...
					if (booking_start_date.getMonth() != booking_end_date.getMonth() && ($('#calender_type').val() != 'time' && $('#calender_type').val() != 'month')) {
						...
						if (!is_reached_time_period(final_date)) {
							...
							select_date(elem, "selected-date cc7");
							// ticket 130688 --Not Allow to select next block when book from one month to next month
							difference_from_to_curr = (new Date(final_date).getTime() - booking_start_date.getTime()) / (1000 * 3600 * 24);
							block_count = Math.ceil((difference_from_to_curr) / book_interval);
							difference_from_to_curr = difference_from_to_curr + block_count;
						...
						//select_date( elem, "selected-date cc8" );
						elem = elem.next();
					}
				}
				i++;
		...
		$(document).on(
			"click",
			'.time-picker-wraper .ph-ul-time .ph-calendar-date:not(.de-active)',
			function () {
				var $allSlots = $('.time-picker-wraper .ph-ul-time .ph-calendar-date');
				var $currentClicked = $(this);

				// If the user clicks a slot that is ALREADY selected (Unselecting)
				if ($currentClicked.hasClass('selected-date')) {
					var clickedIdx = $allSlots.index($currentClicked);

					// Unselect the clicked slot AND everything after it to maintain a valid range
					//$allSlots.slice(clickedIdx).removeClass("selected-date cusClass");
					//console.log(clickedIdx);
					
					if(clickedIdx == 1 && jQuery('.time-picker-wraper .ph-ul-time .ph-calendar-date.selected-date.cusClass').length == 3 ){ // remove class from previous elemento of 2 slot
						$currentClicked.prevAll().removeClass("selected-date cusClass");
					}
					$currentClicked.removeClass("selected-date cusClass");

				}
				else {
					// Standard Range Selection (Selecting)
					var $firstSelected = $allSlots.filter('.selected-date').first();
					if ($firstSelected.length > 0) {
						var firstIdx = $allSlots.index($firstSelected);
						var lastIdx = $allSlots.index($currentClicked);

						if (lastIdx > firstIdx) {
							$allSlots.slice(firstIdx, lastIdx + 1).addClass("selected-date cusClass");
						} else {
							//$allSlots.removeClass("selected-date cusClass");
							$currentClicked.addClass("selected-date cusClass");
							if(lastIdx == 0 && jQuery('.time-picker-wraper .ph-ul-time .ph-calendar-date.selected-date.cusClass').length == 2 && !$currentClicked.next(".ph-calendar-date").hasClass('selected-date') && !$currentClicked.next(".ph-calendar-date").hasClass('de-active') ){
								$currentClicked.nextAll().addClass("selected-date cusClass");
								//console.log("cond-1");
							} else if(lastIdx == 0 && jQuery('.time-picker-wraper .ph-ul-time .ph-calendar-date.selected-date.cusClass').length == 2 && !$currentClicked.next(".ph-calendar-date").hasClass('selected-date') ){
								$currentClicked.nextAll().removeClass("selected-date cusClass");
								//console.log("cond-2");
							}
							//console.log("cond-2");
						}
			...
			function () {
				...
				if ($("#book_interval_type").val() == 'fixed') {
					...
					if (!$is_booking_end_time_clicked && ($is_booking_from_time_clicked || date_from == "" || click % 2 != 0 || $("#book_interval_type").val() != 'customer_choosen')) {
						...
						//$( this ).addClass( "selected-date t1" );
					...
					setTimeout(
						function () {
							...
							while (date_to_obj <= end_date_obj) {
								...
								status = select_date(elem, "selected-date cc2");
								if (!status) {
					...
					setTimeout(
						function () {
							while (end_date_obj > date_to_obj) {
								...
								var elem = jQuery("input[value='" + date_to_select + "']").closest("li")
								status = select_date(elem, "selected-date cc3");
								if (!status) {
									msg_html = elem.find("span").html() + " " + phive_booking_locale.is_not_avail;
									$('.booking-info-wraper').html('<p id="booking_info_text"><span class="not-available-msg">' + msg_html + '</span></p>');
									return false;
